...
-- 프로젝트 종료한것 / 진행중인것 검색
SELECT * FROM PROJECT_TB
WHERE PROJECT_CT_NO IN ( 1, 2, 3, 4, 5 ) AND PROJECT_CLOSE_DT < SYSDATE;

-- 프로젝트 종료한것 STATUS_NO 바꿔주기
UPDATE PROJECT_TB
SET PROJECT_ST_NO = 5
WHERE PROJECT_CLOSE_DT < SYSDATE;

commit;

SELECT P.*, (CASE WHEN PROJECT_NO IN (SELECT LIKE_PRJ FROM LIKE_TB WHERE LIKE_USER = 29) THEN 1 ELSE 0 END) AS ILIKE
		FROM PROJECT_LIST_VIEW_2ND P;


CREATE TABLE "ALARM" (
	"ALARM_NO"	NUMBER		NOT NULL,
	"ALARM_SJ"	VARCHAR2(200)		NOT NULL,
	"ALARM_CONTENT"	VARCHAR2(4000)		NOT NULL,
	"ALARM_STATUS"	VARCHAR2(1)	DEFAULT 'Y'	NOT NULL,
	"ALARM_DATE"	DATE		NOT NULL,
	"ALARM_REF_PNO"	NUMBER		NOT NULL
);

COMMENT ON COLUMN "ALARM"."ALARM_STATUS" IS '안읽었을 시 Y / 읽었을시 N';

COMMENT ON COLUMN "ALARM"."ALARM_DATE" IS '알림 발생 날짜';

COMMENT ON COLUMN "ALARM"."ALARM_REF_PNO" IS '알람 관련 프로젝트 번호';
...
ALTER TABLE "ALARM" ADD CONSTRAINT "FK_PROJECT_TB_TO_ALARM_1" FOREIGN KEY (
	"ALARM_REF_PNO"
)
...
(SELECT P.*, (CASE WHEN PROJECT_NO IN (SELECT LIKE_PRJ FROM LIKE_TB WHERE LIKE_USER = 0) THEN 1 ELSE 0 END) AS ILIKE
FROM PROJECT_LIST_VIEW_2ND P
...
ORDER BY AMOUNT DESC NULLS LAST) B
WHERE ROWNUM < 5;