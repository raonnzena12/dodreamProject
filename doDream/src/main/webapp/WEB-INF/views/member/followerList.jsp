...
<body>
	...
	<div class="row">
		...
		<div class="col-md-8">
			<!-- 팔로잉 리스트 출럭 -->
				<c:forEach var="followerList" items="${ fList }">
				<div class="row my-2">
					<c:if test="${empty fList }">
						리스트가 없습니다.
					</c:if>
				<div class="col-md-3 text-right">
					<c:if test="${ empty fList.followerPfImg }">
					...
					<c:if test="${ !empty fList.followerPfImg }">
					<img alt="프로필사진" src="resources/images/userProfileImage/${fList.followerPfImg }" class="rounded-circle text-center" style="width: 100px; height: 100px;"/>
					</c:if>					
				</div>
				<div class="col-md-6 text-center" style="padding-top: 40px">
					<h6 class="text-left">
						<c:if test="${empty fList.followerName }">
							설정된 닉네임이 없습니다.
						</c:if>
						<c:if test="${! empty fList.followerName }">
						${fList.followerName}
